...
class RnnListener(torch.nn.Module):
    def __init__(self, lang, args):
        ...
        if args.w2v:
            self.emb = nn.Embedding.from_pretrained(args.src_embed,freeze=True, padding_idx=0)
...
class Speaker(torch.nn.Module):
    def __init__(self, lang, args):
        ...
        if args.w2v:
            self.emb = nn.Embedding.from_pretrained(args.trg_embed,freeze=True, padding_idx=0)
        else:
            self.emb = nn.Embedding.from_pretrained(args.trg_embed,freeze=True, padding_idx=0)
